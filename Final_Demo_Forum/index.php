<?php
    ...
    $SQL_hot = "SELECT pNo, tNo, title FROM post WHERE sumView = (SELECT MAX(`sumView`) AS maxnum FROM post)";
    if($result = mysqli_query($link,$SQL_hot)){
        ...
        $SQL_hot_type = "SELECT * FROM type WHERE tNo = '$hot_type_no'";
        if($result1 = mysqli_query($link,$SQL_hot_type)){
    ...
    echo
    "
    <div align='center'><h2>熱門貼文</h2>
        <table class='hot_toppic' frame='border' rules='all' align='center'>
            <tr>
                <td class='hot_toppic_text' ><a href='text.php?pNo=".$hotNo."'><h4> ".$hot_title."</h4></a></th>
            </tr>
        </table>
    </div>

    <div align='center'><h2>主題分類</h2>
        <table class='hot_toppic' frame='border' rules='all' align='center'>
    ";
    ...
    if($result = mysqli_query($link,$SQL_all_type)){
        while($row = mysqli_fetch_assoc($result)){
            ...
            "
                <tr>
                    <td><a href='catalog.php?tNo=".$row["tNo"]."' class='hot_toppic_pic'><img src='".$row["tPic"]."' height='80' alt='logo' class='hot_toppic_pic' style='margin-left: 10px;margin-top: 10px;margin-bottom: 10px'></a></th>
                    <td class='hot_toppic_text' style='padding-right: 20px'><a href='catalog.php?tNo=".$row["tNo"]."'><h4>".$row["tName"]."</h4></a></th>
                </tr>
            ";
        }
    }
